...
// Fixed location ID for this page
$locationId = '5515870852612096';
...
function getLocationDetails($locationId, $token, $base_url, $debug = false) {
    $url = $base_url . '/locations/list' . $locationId;
    
    if ($debug) {
        echo '<div class="alert alert-info">Fetching location details for ID: ' . $locationId . '</div>';
    }
    
    $locationDetails = makeApiRequest($url, $token, null, $debug);
    
    if (!$locationDetails || isset($locationDetails['error'])) {
        // Return a basic location object with the ID if we can't get details
        return [
            'id' => $locationId,
...
// Function to get location details from the locations list
function getSpecificLocationDetails($locationId, $token, $base_url, $debug = false) {
    if ($debug) {
        echo '<div class="alert alert-info">Fetching locations list to find location ID: ' . $locationId . '</div>';
    }
    
    // Get all locations with pagination support
    $locations = getAllLocations($base_url . '/locations/list', $token, $debug);
    ...
    if (!$foundLocation) {
        // Return a basic location object with the ID if we can't find it
        return [
            'id' => $locationId,
            'name' => 'Location ID: ' . $locationId,
            'description' => 'Location details unavailable'
...
// Function to get all locations with pagination support
function getAllLocations($endpoint, $token, $debug = false, $maxPages = 20) {
    ...
    if ($debug) {
        echo '<div class="alert alert-info">Searching for location in page 1...</div>';
    }
    
    // Fetch additional pages as long as there are more and we haven't hit the max
    while ($nextPageToken && $currentPage < $maxPages) {
        if ($debug) {
            echo '<div class="alert alert-secondary">Location not found in current page. Fetching page ' . ($currentPage + 1) . '...</div>';
        }
        ...
        if (is_array($nextPageData)) {
            // If they're simple arrays, just merge them
            if (isset($nextPageData[0])) {
                $allLocations = array_merge($allLocations, $nextPageData);
            }
...
if (!$access_token) {
    ...
    $currentLocation = getSpecificLocationDetails($locationId, $access_token, $base_url, $debug_mode);
}
?>
...
<head>
    ...
    <title>HydroVu - <?php echo isset($currentLocation['name']) ? htmlspecialchars($currentLocation['name']) : 'Well Data'; ?></title>
...
<body>
    <!-- GA4 added 8/22/2025 - Google tag (gtag.js) -->
    ...
    <div class="container mt-4">
        <h1 class="mb-4">
            <?php echo isset($currentLocation['name']) ? htmlspecialchars($currentLocation['name']) : 'HydroVu Well Data'; ?>
        </h1>
        
        <?php if (!$access_token): ?>
            <div class="alert alert-danger">Authentication failed. Please check your client credentials.</div>
        <?php else: ?>
            ...
            <form method="get" class="mb-4">
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <button type="submit" class="btn btn-primary mt-4" id="loadButton">
                ...
                <!-- Hidden field to maintain location ID -->
                <input type="hidden" name="location" value="<?php echo htmlspecialchars($locationId); ?>">
            </form>
            ...
            <?php else: ?>
                ...
                <div class="alert alert-info">
                    ...
                    <p>No data available for this well in the specified time period. This might be because:</p>
                    <ul>
