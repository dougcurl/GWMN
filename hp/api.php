...
function getLocationData($locationId, $token, $base_url, $startTime, $endTime = null, $maxPages = 10, $debug = false) {
    ...
    // Generate cache key
    $cacheKey = "location_{$locationId}_{$startTime}_" . ($endTime ?? 'now');
    $cachedData = getCachedData($cacheKey);
    
    if ($cachedData !== false) {
        if ($debug) {
            echo '<div class="alert alert-info">Using cached data from: ' . date('Y-m-d H:i:s', filemtime(__DIR__ . '/cache/' . md5($cacheKey) . '.cache')) . '</div>';
        }
        return $cachedData;
    }
        ...
        foreach ($week_data['parameters'] as $param) {
            if (isset($param['readings']) && !empty($param['readings'])) {
                ...
                if ($paramKey === 'depth') {
                    ...
                    $readings = transformWaterLevelData($param['readings'], $water_level_baseline);
